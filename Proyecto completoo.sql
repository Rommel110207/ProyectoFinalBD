...
insert into rrhh.Departamento (Nombre_departamento) values 
    (N'Dirección General'), (N'Tecnología'), (N'Ventas'), (N'Marketing (Para borrar)');

insert into rrhh.Puesto (Nombre_puesto) values 
    (N'Gerente General'), (N'Desarrollador Backend'), (N'Ejecutivo de Ventas'), (N'Pasante (Para borrar)');

insert into rrhh.Competencia_objetivo (Nombre, Escala_calificacion) values 
    (N'Liderazgo', 5), (N'Trabajo en Equipo', 5), (N'Resolución de Problemas', 10), (N'Obsolescencia', 5);
...
insert into rrhh.Plantilla_evaluacion (Nombre_plantilla) values 
    (N'Plantilla Ejecutiva'), (N'Plantilla Técnica'), (N'Plantilla Comercial'), (N'Plantilla Borrador');
...
-- UPDATES
update rrhh.Departamento set Nombre_departamento = N'Tecnología e Innovación' where Id_departamento = 2;
update rrhh.Puesto set Nombre_puesto = N'Ingeniero Backend' where Id_puesto = 2;
update rrhh.Competencia_objetivo set Estado = N'Inactivo' where Id_competencia = 3;
update rrhh.Ciclo_evaluacion set Nombre_ciclo = N'Evaluación Anual 2024' where Id_ciclo = 1;
update rrhh.Plantilla_evaluacion set Nombre_plantilla = N'Plantilla TI' where Id_plantilla = 2;
update rrhh.Empleado set Nombres = N'Carlos A.' where Id_empleado = 1;
update rrhh.Evaluacion set Estado = N'Completada', Puntuacion_final = 92.00 where Id_evaluacion = 3;
update rrhh.Detalle_evaluacion set Calificacion = 5 where Id_detalle = 3;

-- DELETES (Físicos y Lógicos)
...
update rrhh.Puesto set Deleted_at = sysdatetime() where Id_puesto = 4;
update rrhh.Departamento set Deleted_at = sysdatetime() where Id_departamento = 4;
go
